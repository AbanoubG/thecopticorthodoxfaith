...
public class ChartDataLogic {
    ...
    public PieData findDueData() {
        ...
        if (dueNumber + notDueNumber > 0) {
            ...
            //Add entries
            labels.add(mApplication.getString(org.copticchurch.library.discoverorthodoxy.R.string.challenge_due_text));
            entries.add(new Entry(dueNumber > 0 ? dueNumber : nullValue(notDueNumber), 0));

            labels.add(mApplication.getString(org.copticchurch.library.discoverorthodoxy.R.string.challeng_not_due_text));
